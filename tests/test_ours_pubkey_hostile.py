"""The hostile-target suite on this branch, with public-key targets.

The same `hostile_ssh_server`, the same modes, the same one-target-per-mode
parametrisation, and the same eight iterations as the upstream comparison. The
single difference from that one is the branch under it — an ordinary public-key
SSH target
here, where the branch's own suite uses a certificate one.
