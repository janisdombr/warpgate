...
Each case runs the same target a hundred times. Ten was what I ran first, and
at one hang in a hundred that could only ever report zero: the expectation over
all seven delays was 0.7 events. A hundred each is seven hundred attempts.
"""
...
def test_how_often_does_this_delay_hang(delay, processes: ProcessManager):
    ...
    try:
        ...
        print(f"RESULT delay={delay:>4} hangs={hangs}/100")
    finally:
        server.stop()
