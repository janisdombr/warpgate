...
# One command per platform, and the raw output of it is kept as evidence next
# to every number derived from it. A parser that quietly matched nothing looks
# exactly like a connection that was never there, and this file's whole point
# is refusing to confuse those two.
...
def _socket_dump() -> str:
    try:
        ...
        return f"<{_SOCKET_CMD[0]} unavailable: {error}>"
    return r.stdout.decode(errors="replace")


def _rows_macos(raw: str):
    """macOS gives Recv-Q, Send-Q, byte counters and the socket buffer
    high-water marks on one line, which is why `netstat` is read rather than
    `lsof`."""
    rows = []
...
def _rows_linux(raw: str):
    """`ss -tanmi`. Linux spreads what macOS puts on one line across three:
    the connection, then `skmem:(...)` for the buffer limits, then the info
    line carrying `bytes_sent`/`bytes_received`. `rb`/`tb` in skmem are
    `sk_rcvbuf`/`sk_sndbuf`, which is what `rhiwat`/`shiwat` mean on the other
    side, and the byte counters are needed because the stall criterion below
    reads them to decide the gateway has gone quiet."""
    rows, cur = [], None
    ...
    for line in raw.splitlines():
        if line[:1] in (" ", "\t"):
            if cur is None:
                continue
            cur["line"] += " | " + line.strip()
            mem = re.search(r"skmem:\(([^)]*)\)", line)
            if mem:
                fields = dict(
                    (k, int(v)) for k, v in re.findall(r"([a-z]+)(\d+)", mem.group(1))
                )
                cur["rhiwat"] = fields.get("rb", 0)
                cur["shiwat"] = fields.get("tb", 0)
            for name, key in (("bytes_sent", "txbytes"), ("bytes_received", "rxbytes")):
                found = re.search(rf"\b{name}:(\d+)", line)
                if found:
                    cur[key] = int(found.group(1))
            continue
        flush()
        f = line.split()
        # The header line and anything else that is not State/Recv-Q/Send-Q.
...
def _run_iteration(
    ...
    with log_path.open("w") as log:
        ...
        try:
            ...
            # A parser that matched nothing and a client that refused to stall
            # both leave `stalled` False, and they mean opposite things: one is
            # a broken instrument, the other a real observation. Told apart by
            # whether any sample was ever taken, and the raw output of whatever
            # this platform's command is goes in the artefacts either way.
            record["socket_samples_taken"] = len(samples)
            if not samples:
                record["instrument"] = (
                    f"{' '.join(_SOCKET_CMD)} produced no row for the "
                    f"127.0.0.1 <-> {wg.ssh_port}/{client_port} pair in "
                    f"{BACKLOG_WAIT_S}s -- the parser, not the client"
                )
                (artifacts / f"raw.{_SOCKET_CMD[0]}.txt").write_text(_socket_dump())
            (artifacts / f"sockets_before_kill.{_SOCKET_CMD[0]}.txt").write_text(
                "\n".join(
                    filter(
                        None,
                        [
            ...
            gateway_row, client_row = _conn_rows(wg.ssh_port, client_port)
            (artifacts / f"sockets_after_observe.{_SOCKET_CMD[0]}.txt").write_text(
                "\n".join(
                    filter(
                        None,
                        [
