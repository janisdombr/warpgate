...
Written against upstream with no part of the certificate feature present: the
target below is an ordinary public-key SSH target whose host happens to answer
with something that is not an SSH version string. Warpgate detects that and
closes its own session; the question is only whether the user's `ssh` is let
go, or left waiting until something kills it.
"""
